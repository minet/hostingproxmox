...
def add_user_dns(user_id, entry, ip):
    ...
    if rep_code == 201:
        ...
        logging.info(
            "DNS entry added: " + str(user_id) + " " + str(entry) + "=> " + str(ip)
        )
...
def delete_from_db(vmid) -> bool:
    try:
        app = (
            util.create_app()
        )  # we need the context to delete the vm if there is an error
...
def delete_from_proxmox(vmid, node) -> bool:
    try:
        ...
        proxmox.nodes(node).qemu(
            vmid
        ).delete()  # need to wait for the deletion but work
...
def create_vm(
    name, vm_type, user_id, cpu, ram, disk, password="no", vm_user="", main_ssh_key="no"
):
    ...
    try:
        ...
        if user is None:
            ...
            database.add_vm(
                id=next_vmid, user_id=user_id, type=vm_type, mac="En attente", ip=ip
            )
            util.subscribe_to_hosting_ML(user_id)
        else:
            util.subscribe_to_hosting_ML(user_id)
            if (
                len(database.get_vm_list(user_id)) < configuration.LIMIT_BY_USER
                and len(database.get_vm_list()) < configuration.TOTAL_VM_LIMIT
            ):
                database.add_vm(
                    id=next_vmid, user_id=user_id, type=vm_type, mac="En attente", ip=ip
                )
        ...
        logging.error(
            "Problem in create_vm(" + str(next_vmid) + ") when cloning: " + str(e)
        )
...
def config_vm(vmid, node, password, vm_user, main_ssh_key, ip, cpu, ram):
    ...
    try:
        ...
        success = False
        logging.error(
            "Problem in create_vm(" + str(vmid) + ") when configuring vm: " + str(e)
        )
        print("Problem in create_vm(" + str(vmid) + ") when configuring vm: " + str(e))
        delete_from_proxmox(vmid, node)
        delete_from_dns(vmid)
        delete_from_db(vmid)
        util.update_vm_state(
            vmid,
            "An error occured while configuring vm (vmid =" + str(vmid) + ")",
            errorCode=500,
        )
    ...
    try:
        ...
        util.update_vm_state(
            vmid,
            "An unkonwn error occured while starting your vm(vmid =" + str(vmid) + ")",
            errorCode=500,
        )
        logging.error(
            "Problem in create_vm(" + str(vmid) + ") when sarting VM: " + str(e)
        )
    ...
    try:
        ...
        proxmox.nodes(node).qemu(vmid).firewall.rules.post(
            action="DROP", type="out", log="nolog", enable=1
        )  # OUT DROP -log nolog
        proxmox.nodes(node).qemu(vmid).firewall.rules.post(
            action="DROP", type="in", log="nolog", enable=1
        )  # IN DROP -log nolog
        proxmox.nodes(node).qemu(vmid).firewall.rules.post(
            action="ACCEPT", type="out", source="+hosting", log="nolog", enable=1
        )  # OUT ACCEPT -source +hosting -log nolog
        proxmox.nodes(node).qemu(vmid).firewall.rules.post(
            action="ACCEPT", type="in", dest="+hosting", log="nolog", enable=1
        ...
        util.update_vm_state(
            vmid,
            "An unkonwn error occured while setting the firewall of your vm(vmid ="
            + str(vmid)
            + ")",
            errorCode=500,
        )
        logging.error(
            "Problem in create_vm("
            + str(vmid)
            + ") when setting the firewall of VM: "
            + str(e)
        )
        print(
            "Problem in create_vm("
    ...
    if success:
        ...
        util.update_vm_state(
            vmid, "An error occured while creating your vm", errorCode=500
        )
...
def start_vm(vmid, node):
    try:
        ...
        logging.error(
            "Problem in start_vm(" + str(vmid) + ") when starting VM: " + str(e)
        )
...
def reboot_vm(vmid, node):
    try:
        ...
        logging.error(
            "Problem in reboot_vm(" + str(vmid) + ") when starting VM: " + str(e)
        )
...
def renew_ip(vmid):
    ...
    try:
        ...
        print(
            "Problem in renew_ip(" + str(vmid) + ") when updating ip address: " + str(e)
        )
        logging.error(
            "Problem in renew_ip(" + str(vmid) + ") when updating ip address: " + str(e)
        )
...
def update_vm_credentials(vmid, username, password, sshKey):
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "Problem while updatig the VM("
                + str(vmid)
                + ") status : "
                + str(e)
            }, 500
        ...
        logging.error(
            "Problem in update_vm_credentials("
            + str(vmid)
            + ") when updating VM: "
            + str(e)
        )
        return {
            "status": "Problem while updatig the VM(" + str(vmid) + ") : " + str(e)
        }, 400
...
def stop_vm(vmid, node):
    try:
        ...
        logging.error(
            "Problem in stop_vm(" + str(vmid) + ") when stopping VM: " + str(e)
        )
...
def get_vm_ip(vmid, node):
    try:
        ips = []
        for int in (
            proxmox.nodes(node).qemu(vmid).agent.get("network-get-interfaces")["result"]
        ):
            if int["name"] == "eth0":
        ...
        logging.error(
            "Problem in get_vm_ip(" + str(vmid) + ") when getting VM infos: " + str(e)
        )
...
def get_vm_hardware_address(vmid, node):
    ...
    return (
        proxmox.nodes(node)
        .qemu(vmid)
        .agent.get("network-get-interfaces")["result"][1]["hardware-address"]
    )
...
def get_vm_name(vmid, node):
    try:
        ...
        logging.error(
            "Problem in get_vm_name(" + str(vmid) + ") when getting VM name: " + str(e)
        )
...
def get_vm(user_id=0, search=None):
    if user_id != 0:  # No filter for non admin
        ...
        if search is None:
            ...
            user_filtered = database.get_user_list(
                searchItem=search
        ...
        for vm in proxmox.cluster.resources.get(type="vm"):
            if vm["vmid"] == vmid:
                try:
                    ...
                    logging.error(
                        "Problem in get_node_from_vm("
                        + str(vmid)
                        + ") when getting VM node: "
                        + str(e)
                    )
...
def get_vm_config(vmid, node):
    ...
    try:
        ...
        logging.error(
            "Problem in get_vm_config(" + str(vmid) + ") when getting VM cpu: " + str(e)
        )
        print(
            "Problem in get_vm_config(" + str(vmid) + ") when getting VM cpu: " + str(e)
        )
    ...
    try:
        ...
        print(
            "Problem in get_vm_config(" + str(vmid) + ") when getting VM ram " + str(e)
        )
        logging.error(
            "Problem in get_vm_config(" + str(vmid) + ") when getting VM ram " + str(e)
        )
    ...
    try:
        ...
        return {
            "error": "An error occured while setting setting the autoreboot option"
        }, 500
    return {
        "name": name,
        "cpu": cpu,
...
def get_vm_current_status(vmid, node):
    try:
        ...
        logging.error(
            "Problem in get_vm_current_status("
            + str(vmid)
            + ") when getting VM global status: "
            + str(e)
        )
    ...
    try:
        ram_usage = round(
            float(current_status["mem"] * 100 / current_status["maxmem"]), 1
        )
    except:
        logging.error(
            "Problem in get_vm_current_status("
            + str(vmid)
            + ") when getting VM ram usage: "
            + str(e)
        )
...
def get_proxmox_vm_status(vmid, node):
    ...
    try:
        ...
        logging.error(
            "Problem in get_proxmox_vm_status("
            + str(vmid)
...
def check_dns_ip_entry(user_id, ip: str) -> bool:
    try:
        ...
        if ip_list == None:
            print(
                "ERROR : the vm list of user ", user_id, " failed to be retrieved : ", e
            )
            return None
        isOk = ip in ip_list
        if not isOk:
            print(
                "INCIDENT REPORT : the user",
                user_id,
                " tried to set up a DNS entry for ip",
                ip,
                "he doesn't own.",
            )
        return isOk
...
def get_freeze_state(username):
    ...
    if user is None:
        return {
            "freezeState": "0"
        }, 200  # user doesn't exist so we fake the freezestatus
    try:
        ...
        return {
            "freeztatus": "unknown"
        }, 404  # User doesn't exist, we fake the freeze state to 0.0
    if freezeState is None:  # We have to create the freeze state
        ...
        freezeState = database.getFreezeState(
            username
        )  # if expired with update in case of re-cotisation
        status = freezeState.split(".")[0]
        return {"freezeState": status}, 200
...
def next_available_vmid():  # determine the next available vmid from both db and proxmox
    ...
    while (
        next_vmid_db != None and not is_vmid_available_prox
    ):  # if next_vmid_db is None then there is no next vmid available and if is_vmid_available_prox = True then the next vmid is available in proxmox and in db
        next_vmid_db += 1
