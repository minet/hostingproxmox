...
def create_dns(body=None):  # noqa: E501
    ...
    if "attributes" in cas:
        if "memberOf" in cas["attributes"]:
            if is_admin(
                cas["attributes"]["memberOf"]
            ):  # partie admin pour renvoyer l'owner en plus
                admin = True
    ...
    if not util.check_dns_entry(body.entry):
        print(
            "INCIDENT REPORT : The user ( id ",
            str(user_id),
            ") overrided frontend security to submit a forbidden DNS entry : ",
            body.entry,
            "for ip",
            body.ip,
        )
        return {
            "error": "This DNS entry is forbidden. This incident will be reported."
        }, 403

    isOk = proxmox.check_dns_ip_entry(user_id, body.ip)
    if isOk is None:
        return {
            "error": "An error occured while checking your ip addresses. Please try again."
        }, 500
    elif not isOk and not admin:
        return {
            "error": "This ip address isn't associated to one of your vms. This is illegal. This incident will be reported."
        }, 403
    elif admin:
...
def create_vm(body=None):  # noqa: E501
    ...
    if (
        freezeAccountState != 0 and not admin
    ):  # for any other freestate user can't create vm
        return {"error": "Your cotisation has expired"}, 403
    ...
    try:
        ...
        return {
            "error": "Impossible to create a VM without CPU, RAM or disk. No data transmitted"
        }, 400
    ...
    if (
        alreadyUsedCPU + body.cpu > 6
        and alreadyUsedRAM + body.ram > 8
        and alreadyUsedDisk + body.disk > 30
    ):
...
def delete_vm_id_with_error(vmid):  # API endpoint to delete a VM when an error occured
    ...
    user_id = cas["sub"]
    if (
        not admin and dbfct.get_vm_userid(vmid) != user_id
    ):  # if not admin, we check if the user is the owner of the vm
...
def delete_vm_id(vmid):  # noqa: E501
    ...
    if (
        freezeAccountState >= 3 and not admin
    ):  # if freeze state 1 or 2 user still have access to proxmox
    ...
    try:
        if node == "" and not dueToError:
            ...
            util.update_vm_state(
                vmid,
                "Impossible to find the vm to delete.",
                errorCode=404,
                deleteEntry=True,
            )
            return 0
            ...
            if not isProxmoxDeleted and not dueToError:
                ...
                util.update_vm_state(
                    vmid,
                    "An error occured while deleting the VM from proxmox",
                    errorCode=500,
                    deleteEntry=True,
                )
                return 0
        ...
        if not isDNSDeleted and not dueToError:
            ...
            util.update_vm_state(
                vmid,
                "An error occured while deleting the DNS entry",
                errorCode=500,
                deleteEntry=True,
            )
            return 0
        isDbDeleted = proxmox.delete_from_db(vmid)
        if (not dueToError and isDbDeleted and isProxmoxDeleted) or (
            dueToError and (isDbDeleted or not isProxmoxDeleted)
        ):
            ...
            util.update_vm_state(
                vmid,
                "An error occured while deleting the VM.",
                errorCode=500,
                deleteEntry=True,
            )
    except Exception as e:
        ...
        util.update_vm_state(
            vmid,
            "An error occured while deleting the VM. Exception : " + str(e),
            errorCode=500,
            deleteEntry=True,
        )
...
def get_dns():  # noqa: E501
    ...
    if (
        freezeAccountState >= 3 and not admin
    ):  # For freeze state 1 and 2, the user still can be connected to hosting
        return {"status": "cotisation expired"}, 403
...
def get_vm(search=""):  # noqa: E501
    ...
    if (
        freezeAccountState >= 3 and not admin
    ):  # For freeze state 1 or 2, the user can access to hosting
        return {"error": "cotisation expired"}, 403
        ...
        if "memberOf" in cas["attributes"]:
            if is_admin(cas["attributes"]["memberOf"]):
                return proxmox.get_vm(
                    search=search
                )  # affichage de la liste sans condition
...
def get_vm_id(vmid):  # noqa: E501
    ...
    if (
        vm_state != None
    ):  # if not then the vm is created of not found. Before get the proxmox config, we must be sure the vm is not creating or deleting
        (status, httpErrorCode, errorMessage) = vm_state
        if status == "error":
            util.update_vm_state(
                vmid, "delete", deleteEntry=True
            )  # We send to the user and delete here
            try:
                return {"error": errorMessage}, int(httpErrorCode)
            except:
                return {"error": "An unknown error occured"}, 500
        elif (
            not vmid in map(int, proxmox.get_vm(user_id)[0]) and not admin
        ):  # we authorize to consult error message
            return {"error": "You don't have the right permissions"}, 403
    ...
    if (
        ...
        status != 200 and not admin
    ):  # exist in the db but not in proxmox. It's a error
    ...
    elif response == 200 or (
        response == 201 and "status" in vmConfig.keys()
    ):  # If the returned code is 201 and the vm is just created
    ...
    owner = get_vm_userid(
        vmid
    )  # on renvoie l'owner pour que les admins puissent savoir à quel user appartient quelle vm

    if status[0]["status"] != "running":
        return {
            "name": name,
            "autoreboot": autoreboot,
            "user": owner if admin else "",
            "ip": "",
    ...
    if (status[1] == 201 or status[1] == 200) and (type[1] == 201 or type[1] == 200):
        ...
        print(
            "datal error for vm ",
            vmid,
            "Unknown error one of the status, type or ip doesn't exists : ",
            status,
            type,
            ip,
        )
        return {
            "error": "Unknown error one of the status, type or ip doesn't exists."
        }, 500
...
def delete_dns_id(dnsid):  # noqa: E501
    ...
    user_id = cas["sub"]
    if (
        not admin and dbfct.get_entry_userid(dnsid) != user_id
    ):  # if not admin, we check if the user is the owner of the vm
        return {"error": "Forbidden"}, 403
    if admin:
        freezeAccountState = 0
...
def patch_vm(vmid, body=None):  # noqa: E501
    ...
    if (
        admin or dbfct.get_vm_userid(vmid) == user_id
    ):  # if not admin, we check if the user is the owner of the vm
        node, status = proxmox.get_node_from_vm(vmid)
        if status != 200:
            return {"status": "vm not exists"}, 404
...
def get_ip_list():
    ...
    if list == None:
        return {
            "status": "Impossible to retrieve the list of your ip addresses. Please make juste you have at least one."
        }, 500
    else:
...
def update_credentials():
    ...
    if (
        not "password" in update_body
        or not "sshKey" in update_body
        or not "username" in update_body
    ):
        return {
            "status": "Missing parameters. You must provide a username, a password and a valid public ssh key"
        }, 400

    if not util.check_password_strength(update_body["password"]):
        return {
            "status": "Incorrect password format. Your password must contain at least 1 special char, 1 uppercase letter, 1 number and 8 chars in total."
        }, 400
    ...
    return proxmox.update_vm_credentials(
        vmid, update_body["username"], update_body["password"], update_body["sshKey"]
    )
