...
def test_creation_for_new_user_10G(
    monkeypatch, init_user_database, init_vm_database, client
):
    ...
    with client:
        ...
        monkeypatch.setattr(
            util, "subscribe_to_hosting_ML", fake_subscribe_to_hosting_ML
        )
...
"""
        ...
        monkeypatch.setattr(
            proxmox, "check_update_cotisation", fake_check_update_cotisation
        )
...
def test_creation_for_new_user_20G(
    monkeypatch, init_user_database, init_vm_database, client
):
    """Test case for creation of VM by a new user
    The creation does not concern proxmox
...
def test_creation_for_new_user_30G(
    monkeypatch, init_user_database, init_vm_database, client
):
    """Test case for creation of VM by a new user
    The creation does not concern proxmox
...
def test_creation_for_new_user_incorrect_storage(
    monkeypatch, init_user_database, init_vm_database, client
):
    """Test case for creation of VM by a new user
    The creation does not concern proxmox
